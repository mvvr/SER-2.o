...
def main():
    ...
    with st.container():
        ...
        with col1:
            ...
            if audio_file is not None:
                ...
                if st.button("Try test file"):
                    wav, sr = librosa.load("audios/audio3.wav", sr=44100)
                    Xdb = get_melspec("audios/audio3.wav")[1]
                    mfccs = librosa.feature.mfcc(wav, sr=sr)
                    # display audio
                    st.audio("audios/audio3.wav", format='audio/wav', start_time=0)
                    path = "audios/audio3.wav"
                    audio_file = "test"
        with col2:
            if audio_file is not None:
        ...
        em3 = st.checkbox("3 emotions", True)
        em6 = st.checkbox("6 emotions", True)
        em7 = st.checkbox("7 emotions")
